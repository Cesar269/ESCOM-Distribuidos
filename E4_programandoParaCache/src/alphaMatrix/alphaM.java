...
public class alphaM {
    ...
    public static void calcularC4(long[][] M1, long[][] M2){
        //C4 = A2 x B2
        ...
        for (int i = N/2; i < N; i++)
            for (int j = N/2; j < N; j++)
                for (int k = 0; k < N; k++){
                    //System.out.println("M1["+i+"]["+k+"]*M2["+j+"]["+k+"]= "+M1[i][k] +"*"+ M2[j][k]+"="+M1[i][k] * M2[j][k]);
                    C4[i][j] += M1[i][k] * M2[j][k];
                }
        //printMatrix(C4,"C4");
        //C4
        for (int i=N/2; i<N; i++)
